package ws.outset.utils {
	...
	import ws.outset.managers.menu.MenuManager;
	...
	public class Utils {
		...
		public static function scaleByPoint(movie:DisplayObject, scale:Number, point:Point, animate:Boolean = true, steps:int = 20, offsetX:Number = 0, offsetY:Number = 0):void {	
			var newW:Number;
			var newH:Number;
			...
			var newX:Number;
			var newY:Number;
			var distX:Number = ((point.x - movie.x) / movie.width);
			var distY:Number = ((point.y - movie.y) / movie.height);
			if (movie.scaleX != 1 && movie.scaleY != 1) {
				normalWidth = movie.width / movie.scaleX;
				normalHeight = movie.height / movie.scaleY;
			} else {
				normalWidth = movie.width;
				normalHeight = movie.height;
			}
			newW = normalWidth * scale;
			newH = normalHeight * scale;
			newX = point.x - (distX * newW);
			newY = point.y - (distY * newH);			
			if (animate) {
				TweenManager.tween(
					movie,
					{
						scaleX: { end:scale, mtd:TweenType.OUT_QUAD, steps:steps },
						scaleY: { end:scale, mtd:TweenType.OUT_QUAD, steps:steps },
						x: { end:newX + offsetX, mtd:TweenType.OUT_QUAD, steps:steps },
						y: { end:newY + offsetY, mtd:TweenType.OUT_QUAD, steps:steps }						
					}
				);
			} else {
				movie.scaleX = movie.scaleY = scale;
				movie.x = newX + offsetX;
				movie.y = newY + offsetY;
			}
		}
		...
		public static function addCopyright(movie:*, label:String, link:String):void {
			var menu:MenuManager = new MenuManager(movie);
			menu.addItem(label, link);
		}
		/**
